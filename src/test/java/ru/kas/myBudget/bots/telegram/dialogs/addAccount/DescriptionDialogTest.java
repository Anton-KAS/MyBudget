...
public class DescriptionDialogTest extends AbstractDialogImplTest {
    ...
    @ParameterizedTest
    @MethodSource("sourceReturnFalseBankCommit")
    public void shouldProperlyExecuteCommit(Update update, boolean expected) {
        ...
        if (!expected && !update.hasCallbackQuery()) {
            Mockito.verify(botMessageServiceMock).executeAndUpdateUser(telegramUserServiceMock, update, ExecuteMode.SEND,
                    String.format(VERIFY_EXCEPTION_TEXT, MAX_DESCRIPTION_LENGTH), keyboardMock.getKeyboard());}
        if (expected) Mockito.verify(telegramUserServiceMock).checkUser(telegramUserServiceMock, update);

    }

    public static Stream<Arguments> sourceReturnFalseBankCommit() {
        return Stream.of(
